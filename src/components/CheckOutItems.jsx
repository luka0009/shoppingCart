...
export default function CheckOutItems({cartItem}) {
  ...
  return (
    <div className='bg-white flex justify-between items-center border border-solid
    border-glass p-4 mb-6'>
        <div className='flex items-center gap-4'>
            <img src={image} alt="" className='w-32 h-20 object-cover'/>
        </div>
        <div className="flex flex-col items-start max-w-[6.8rem]">
        <div>{name}</div>
        <div className="flex items-center gap-4 mt-2">
          <button
            className="w-8 h-8 text-white bg-black hover:opacity-90 rounded-full "
      ...
      <div className="flex flex-col items-center gap-3">
        <HiX
          className="cursor-pointer text-2xl hover:opacity-75"
